...
const { ApiError } = require('../middleware/errorHandler');
const { Op } = require('sequelize');
const sequelize = require('../config/sequelize'); // ⬅ IMPORTANT (needed for stats query)
...
class AttendanceRepository {
  ...
  // SQL QUERY:
  // INSERT INTO attendance (employee_id, date, status, is_active, total_hours, clock_in, clock_out)
  // VALUES (...);
  // ============================================================
  async create(attendanceData) {
    try {
  ...
  // FIND BY ID
  // SQL QUERY:
  // SELECT * FROM attendance WHERE id = :attendanceId LIMIT 1;
  // ============================================================
  async findById(attendanceId) {
    try {
      console.log('Finding attendance by ID:', attendanceId);
  ...
  // WHERE employee_id = :id AND is_active = TRUE AND status = 'active'
  ...
  //
  // SELECT COUNT(*) FROM attendance WHERE <filters>;
  // ============================================================
  async findWithPagination(filters = {}, options = {}) {
    try {
      const {
